...
function assertProductionChromiumAuthConfig(options: {
  ...
  const missing = [
    !options.crxPublicKey && 'CRX_PUBLIC_KEY',
...
export default defineConfig({
  srcDir: 'src',
  filterEntrypoints: ['background', 'content', 'popup'],
  outDirTemplate: '{{browser}}-mv{{manifestVersion}}{{modeSuffix}}',
  zip: {
    name: 'brute-bookmarks-quick-save',
  },
  manifest: ({ browser, mode }) => {
    ...
    return {
      name: 'BruteBookmarks Quick Save',
      description: 'Save pages to BruteBookmarks from any tab, with or without an account.',
      ...(browser === 'chrome' && crxPublicKey ? { key: crxPublicKey } : {}),
      permissions,
      host_permissions: Array.from(hostPermissions),
      icons: {
        16: '/cws-quicksave-icon-16.png',
        32: '/cws-quicksave-icon-32.png',
        48: '/cws-quicksave-icon-48.png',
        128: '/cws-quicksave-icon-128.png',
        300: '/cws-quicksave-icon-300.png',
      },
      action: {
        default_title: 'BruteBookmarks Quick Save',
        default_icon: {
          16: '/quicksave-icon-16.png',
          32: '/quicksave-icon-32.png',
          48: '/quicksave-icon-48.png',
          128: '/quicksave-icon-128.png',
        },
      },
    };
  },
