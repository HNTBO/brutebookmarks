...
for (const [glyph, prefix] of [
  ['quicksave', 'cws-quicksave-icon'],
  ['newtab', 'cws-newtab-icon'],
]) {
  for (const size of SIZES) {
    const rgba = renderIcon(glyph, size);
    writeFileSync(join(OUT_DIR, `${prefix}-${size}.png`), encodePng(size, size, rgba));
  }
}
